...
fn benchmark_throughput_latency() {
    ...
    let mut logger = Logger::<Data>::new(path.to_string(), 4096, 10_000_000).unwrap();
    ...
    while start_bench.elapsed() < run_duration {
        ...
        let seq_id = logger.log(Data { val: count, ts: 0, padding: [0; 84] });
    ...
    let d_p50 = if d_len > 0 { disk_latencies[(d_len as f64 * 0.50) as usize] } else { Duration::ZERO };
    let d_p95 = if d_len > 0 { disk_latencies[(d_len as f64 * 0.95) as usize] } else { Duration::ZERO };
    let d_p99 = if d_len > 0 { disk_latencies[(d_len as f64 * 0.99) as usize] } else { Duration::ZERO };

    println!("Total Ops: {}", count);
    println!("Duration: {:.2?}", total_duration);
    println!("Throughput: {:.2} Op/s", count as f64 / total_duration.as_secs_f64());
